...
function validate(e: Event) {
  ...
  function setErrorSubject(err: string) {
  ...
  // Check name
  if (name.trim() == "") setErrorName("Name is required.");
  else if (!/^[a-zA-Z ]+$/.test(name)) setErrorName("Name can only include letters and spaces.");
  
  // Check subject
  if (subject.trim() == "") setErrorSubject("Subject is required.");
  
  // Check email
  if (mail.trim() == "") setErrorMail("E-Mail is required.");
  ...
  // Check message
  if (message.trim() == "") setErrorMessage("Message is required.");
  if (message.length <= 32) setErrorMessage("Message is too short, at least 32 characters required.");
  if (message.length > 1000) setErrorMessage("Message is long, maximum 1000 characters.");
